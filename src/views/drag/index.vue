 ...
 * @version: 
 * @Author: 周涛
 * @Date: 2021-09-22 16:37:32
 * @LastEditors: 周涛
 * @LastEditTime: 2021-09-22 17:33:24
-->
<template>
  <div class="drag-list">
    <el-card header="拖拽列表">
              ...
              <transition-group class="drag-wrapper" tag="ul" name="flip-list">
                ...
                >
                  <div>
                    <svg-icon
                      @click="fixed(todo)"
                      class-name="todo-icon"
                      :icon-class="todo.fixed ? 'fixed' : 'drag'"
                    ></svg-icon>
                    {{ todo.name }}
                  </div>
                  <div>
...
export default {
  ...
  methods: {
    ...
    handleChange({ $event, value }, type) {
      ...
      this.history.push(
        `${from} => ${to} ${$event.oldIndex} => ${$event.newIndex} `
      );
    },
    fixed(item) {
